...
- (void)viewDidLoad {
    ...
    NSString *queryFeed = [NSString stringWithFormat:@"%@/iosStreamv2/biology/BiologyvideoiGCSECamb.xml",domain];
    NSString *Dir = [appDelegate.applicationDocumentsDirectory stringByAppendingPathComponent:@"BiologyvideoiGCSECamb.xml"];
...
-(void)MyParser:(NSString *)FileLocation{
	...
	for(int idx = 0; idx < pointStrings.count - 1; idx++)
	{
        ...
        obj.ProductID = @"BiologyiGCSECambiPad";
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if(section == 0){
        ...
        if([filteredArrayofConfigObjects count] > 0 && FullSubscription == FALSE ){
            sectionHeader.text = @"iGCSE Biology Cambridge – Start today from only £1.49";
        }
        else if ([filteredArrayofConfigObjects count] > 0 && FullSubscription == TRUE ){
            sectionHeader.text = @"My Courses – iGCSE Biology Cambridge";
        }
        else{
            sectionHeader.text =@"";
        }
